...
export default class Colour {
   ...
   */
  static rgba2lab(r: number, g: number, b: number, a = 1) {
    const [x, y, z] = Colour.rgb2xyz(r, g, b, a);
    return Colour.xyz2lab(x, y, z); // [l, a, b]
  }
  /**
   ...
   * @param {[Number]} b
   */
  static lab2rgba(l: number, a: number, b: number) {
  ...
  static hex2rgba(hex: string) {
    let c = hex.charAt(0) === "#" ? hex.substring(1).split("") : hex.split("");
   ...
   */
  static rgb2xyz(r: number, g: number, b: number, a = 1) {
    if (r > 255) {
   ...
   * @param {[Number]} z
   */
  static xyz2rgba(x: number, y: number, z: number): [number, number, number, 1] {
    let varX = x / 100;
    let varY = y / 100;
    let varZ = z / 100;
   ...
   * @param {[Number]} b     Blue value from 0 to 255
   */
  static getDarkerColour(
    r: number,
    g: number,
    b: number,
    a = 1,
    darkenPercentage = 0.05
  ) {
   ...
   * @param {[Number]} b     Blue value from 0 to 255
   */
  static getBrighterColour(
    r: number,
    g: number,
    b: number,
    a = 1,
    brighterPercentage = 0.05
  ) {
    let [l1, a1, b1] = Colour.rgba2lab(r, g, b, a);
